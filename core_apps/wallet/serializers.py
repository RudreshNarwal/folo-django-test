...
class ResidentialAddressSerializer(serializers.Serializer):
    """
    Serializer for the residential_address nested field within customer creation.
    """
    street_line_1 = serializers.CharField(max_length=255)
    city = serializers.CharField(max_length=100)
    subdivision = serializers.CharField(max_length=100) # State or province
    postal_code = serializers.CharField(max_length=20)
    country = serializers.CharField(max_length=100)


class IdentifyingInformationSerializer(serializers.Serializer):
    """
    Serializer for individual identifying information objects.
    Note: 'image_front' and 'image_back' are base64 strings if used.
    """
    type = serializers.CharField(max_length=50) # e.g., "ssn", "drivers_license", "passport"
    issuing_country = serializers.CharField(max_length=100)
    number = serializers.CharField(max_length=255)
    image_front = serializers.CharField(required=False, allow_blank=True, help_text="Base64 encoded image for front of ID.")
    image_back = serializers.CharField(required=False, allow_blank=True, help_text="Base64 encoded image for back of ID.")
...
class CreateCustomerSerializer(serializers.Serializer):
    ...
    """
    type = serializers.CharField(max_length=50) # "individual" or "business"
    first_name = serializers.CharField(max_length=100)
    last_name = serializers.CharField(max_length=100)
    email = serializers.EmailField()
    residential_address = ResidentialAddressSerializer() # Nested serializer
    birth_date = serializers.DateField(format="%Y-%m-%d") # Expects YYYY-MM-DD format
    signed_agreement_id = serializers.CharField(max_length=255, help_text="ID obtained from TOS acceptance.")
    identifying_information = serializers.ListField(
        child=IdentifyingInformationSerializer(),
        required=False, # Often optional at initial creation or depends on country
        allow_empty=True
    )

    def validate(self, data):
        """
        Custom validation to ensure that identifying information is provided
        if the customer type is 'individual'.
        """
        if data['type'] == 'individual' and not data.get('identifying_information'):
            raise serializers.ValidationError({
                "identifying_information": "Identifying information is required for individual customers."
            })
