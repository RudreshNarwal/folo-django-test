...
from core_apps.common.services import APILoggingService
...
if __name__ == "__main__":
    ...
    MY_BRIDGE_API_KEY = settings.BRIDGE_API_KEY

    # Example of a redirect URI for TOS acceptance
    MY_REDIRECT_URI = "https://your-app.com/tos-callback"

    # Example customer data (replace with actual data for your tests)
    EXAMPLE_CUSTOMER_DATA = {
        ...
        "residential_address": {
            "street_line_1": "123 Main St",
            "city": "Anytown",
            "subdivision": "CA",  # State abbreviation
            "postal_code": "90210",
            "country": "USA"
        },
        "birth_date": "1990-05-15",
        "signed_agreement_id": "",  # IMPORTANT: This needs to be populated from TOS link acceptance
        "identifying_information": [
            {
                "type": "ssn",
                "issuing_country": "usa",
                "number": "000-00-0000"  # Use test SSN or dummy for testing
            }
        ]
    }

    # Example transfer data (replace with actual data for your tests)
    # This example mimics the "off-ramp USDC into a fiat bank account via wire" from the PDF
    EXAMPLE_TRANSFER_DATA = {
        "amount": "100.00",
        "on_behalf_of": "customer_123",  # Replace with an actual customer ID
        "source": {},
        "payment_rail": "polygon",
        "currency": "usdc",
        "from_address": "0xdeadbeef",  # Replace with a valid crypto address
        "destination": {
            "payment_rail": "wire",
            "currency": "usd",
            "external_account_id": "external_account_123"  # Replace with a registered external account ID
        }
    }

    try:
        # Initialize the service
        bridge_service = BridgeAPIService(api_key=MY_BRIDGE_API_KEY)
        print("BridgeAPIService initialized successfully.")

        # --- 1. Request Terms of Service Link ---
        print("\n--- Requesting Terms of Service Link ---")
        try:
            tos_link_response = bridge_service.request_terms_of_service_link(redirect_uri=MY_REDIRECT_URI)
            tos_url = tos_link_response.get("url")
            print(f"Terms of Service URL: {tos_url}")
            print("Please open this URL in a browser for the customer to accept the terms.")
            print(
                "After acceptance, the customer will be redirected to your `redirect_uri` with a `signed_agreement_id`.")
            print("You will need to manually get this `signed_agreement_id` and update EXAMPLE_CUSTOMER_DATA.")

            # For demonstration, we'll assume a dummy ID is obtained.
            # In a real app, this would come from the redirect callback.
            dummy_signed_agreement_id = "mock_signed_agreement_id_12345"
            EXAMPLE_CUSTOMER_DATA["signed_agreement_id"] = dummy_signed_agreement_id
            print(f" (Using dummy signed_agreement_id: {dummy_signed_agreement_id} for subsequent steps)")

        except BridgeAPIError as e:
            print(f"Error requesting TOS link: {e.message}")
            if e.response_data:
                print(f"API Response: {e.response_data}")
        except requests.exceptions.RequestException as e:
            print(f"Network error requesting TOS link: {e}")

        # --- 2. Create a Customer ---
        print("\n--- Creating Customer ---")
        if EXAMPLE_CUSTOMER_DATA["signed_agreement_id"]:  # Only proceed if signed_agreement_id is set
            try:
                # IMPORTANT: Replace SSN/DL details with test data or remove if not testing identifying_information
                # Note: KYC status will be returned in the response
                new_customer = bridge_service.create_customer(EXAMPLE_CUSTOMER_DATA)
                print("Customer created successfully:")
                print(json.dumps(new_customer, indent=2))
                # Save the customer ID for subsequent calls like transfers
                # EXAMPLE_TRANSFER_DATA["on_behalf_of"] = new_customer.get("id")
            ...
            print("Cannot create customer without a signed_agreement_id. Please obtain it first.")

        # --- 3. Initiate a Transfer ---
        print("\n--- Initiating Transfer ---")
        # Ensure 'on_behalf_of' and 'external_account_id' are valid from your Bridge dashboard
        # and Plaid integration before running this.
        if EXAMPLE_TRANSFER_DATA["on_behalf_of"] != "customer_123" and \
                EXAMPLE_TRANSFER_DATA["external_account_id"] != "external_account_123":
            try:
                transfer_response = bridge_service.initiate_transfer(EXAMPLE_TRANSFER_DATA)
                print("Transfer initiated successfully:")
                print(json.dumps(transfer_response, indent=2))
            except BridgeAPIError as e:
                print(f"Error initiating transfer: {e.message}")
                if e.response_data:
                    print(f"API Response: {e.response_data}")
            except requests.exceptions.RequestException as e:
                print(f"Network error initiating transfer: {e}")
        else:
            print(
                "Skipping transfer: Please update `on_behalf_of` and `external_account_id` in `EXAMPLE_TRANSFER_DATA` with real IDs from your Bridge account setup.")


    except ValueError as ve:
        print(f"Service Initialization Error: {ve}")
